...
4. **No interactive slot shell for agents**: Do not use `git slot shell` or rely on `launch_shell` for agent-driven flows (TTY / stdin issues). For slot-scoped env without a subshell, see §2.1 — **`git slot exec`** is optional, not mandatory, when plain `cd` + commands are enough.
...
Optional helpers for one-shot commands: they set the slot **working directory** and merge **environment** (`GSL_*` plus `[[slots.env]]`) without `GSL_SHELL_SESSION` and without a TTY-consuming subshell.

- **`git slot exec <slot> -- <command> [args...]`** — Run inside that slot's worktree with merged env. Example: `git slot exec main-work -- npm test`
- **`git slot exec -- <command> [args...]`** — Slot is inferred from the current directory (same rule as `which`). **When your cwd is already inside the correct slot worktree and that matches the task's slot, this is a reasonable default wrapper** so env stays aligned with config; if the command does not care about `[[slots.env]]` / `GSL_*`, running it directly after `cd` is fine too.
- **`git slot which`** — Prints the configured slot name for the current git worktree root (one line). Handy when you need the slot name in a script; exits non-zero if the cwd is not a slot worktree.

**Syntax**: `--` is required before the command (e.g. `exec -- npm run build`, not `exec npm run build`).
...
### Step 3: Execute Task
Perform all file edits, script executions, and tests strictly within this slot's directory. After `cd` into the slot, you can run tools normally, or wrap invocations with **`git slot exec -- …`** when you want slot env applied consistently — neither style is universally required; pick what fits the task.
