...
4. **Slot env without shells**: When `[[slots.env]]` or `GSL_*` variables must apply (e.g. `PORT`, `GSL_SLOT_NAME`), use **`git slot exec`** (see §2.1). Do not rely on `git slot shell` or `launch_shell` for agents.
...
Use these for one-shot commands with the correct slot **working directory** and **environment** (`GSL_*` plus `[[slots.env]]`). They do **not** set `GSL_SHELL_SESSION` and do not open a TTY-consuming subshell.

- **`git slot exec <slot> -- <command> [args...]`** — Run `<command>` inside that slot's worktree with merged env. Example: `git slot exec main-work -- npm test`
- **`git slot exec -- <command> [args...]`** — Same, but the slot is inferred from the current directory (same rule as `which`). Use after `cd` into the slot tree, or from any path if you pass `<slot>` explicitly.
- **`git slot which`** — Prints the configured slot name for the current git worktree root (one line). Use when you need the slot name in a script; exits non-zero if the cwd is not a slot worktree.
...
### Step 3: Execute Task
Perform all file edits, script executions, and tests strictly within this slot's directory. Prefer **`git slot exec`** (from repo root with an explicit slot, or from the slot tree with `exec -- …`) when the task depends on slot-specific environment variables; otherwise `cd` into the slot path and run commands normally. 
